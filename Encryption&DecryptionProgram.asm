...
#addi $v1, $0, 0 # this also works
li $v1, 0 # 
	loop:
		
		slt $s7, $v1, $s1               # compare counter to total number of characters in message
		beq $s7,$0, exitProcedure # if s7 is 1 go to exitprocedure, else continue
		
		add $t1, $a0, $v1 # incretementing the pointer towards the address based on current value of counter
		lbu $t2,0($t1) # loading the byte 
		
		blt $t2, 65, doneEncrypting
		bgt $t2, 90, doneEncrypting
		beq $t2, 32, doneEncrypting # Optional
		beq $t2, 33, doneEncrypting
		beq $t2, 39, doneEncrypting
		
		add $t2,$t2, $a1 # add the key to the specific byte 
		
		blt $t2,65,lessThan 
		bge $t2,91,greaterThan
...
li $v1, 0
	loop2:
		slt $s7, $v1, $s1               # compare counter to total number of characters in message
		beq $s7,$0, exitProcedure2 # if s7 is 1 go to exitprocedure, else continue
		...
		blt $t2, 65, doneEncrypting
		bgt $t2, 90, doneEncrypting
		beq $t2, 32, donedecrypting
		beq $t2, 33, donedecrypting
		beq $t2, 39, donedecrypting
		
		mul $a1,$a1,-1 
		add $t2,$t2,$a1 # add the key to the specific byte 
		#sub $t2, $t2, $a1 # should also work but I prefer the method used
		
		blt $t2,65,lessThan2
		bge $t2,91,greaterThan2
